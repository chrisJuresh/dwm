...
static const char dmenufont[]       = "monospace:pixelsize=15";
...
void togglelayout(const Arg *arg) {
    (void)arg;
    if (selmon->lt[selmon->sellt] == &layouts[0]) // If current layout is layouts[0]
        setlayout(&((Arg) { .v = &layouts[1] })); // Set layout to layouts[1]
    else
        setlayout(&((Arg) { .v = &layouts[0] })); // Otherwise, set it to layouts[0]
}
...
/* button definitions */
/* click can be ClkTagBar, ClkLtSymbol, ClkStatusText, ClkWinTitle, Cwhich keypresses to restart dwmlkClientWin, or ClkRootWin */
